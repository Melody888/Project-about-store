<template>
  <mt-popup v-model="visible" position="bottom" class="bm mint-datetime">
    <mt-picker :slots="dateSlots" @change="onChange" :visible-item-count="visibleItemCount" class="mint-datetime-picker" ref="picker" show-toolbar>
      <span class="mint-datetime-action mint-datetime-cancel" @click="cancel">{{ cancelText }}</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getValue (values) {
        ...
        if (this.type === 'time') {
          ...
          let date = this.typeStr.indexOf('D') > -1 ? this.getTrueValue(values[2]) : 1
      ...
      fillValues (type, start, end) {
        ...
        let m = []
        if (this.timePickerIncrement !== 1 && type === 'm') {
          for (var i = 0; i < 59; i++) {
            if (i % this.timePickerIncrement === 0) {
              m.push(i)
            }
          }
        }
    ...
    watch: {
      ...
      startDate (val) {
        if (val && this.currentValue < val) this.currentValue = val
      },
      rims () {
        this.generateSlots()
      }
