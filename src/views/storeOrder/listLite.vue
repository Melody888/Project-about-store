<template>
  <bm-layout class="m_store-order-list" ref="bmLayout">
    ...
    <section class="m_content">
      <div class="m_main">
        <bm-loadmore class="loadmore" v-scrolled="'storeOrderLiteHandle'" :top-method="loadList" ref="loadList_1"  v-show="tab === 'STORE'">
          <ul
              v-infinite-scroll="fetchHandleOrderList"
              infinite-scroll-disabled="handleOrderListScrollDisabled"
              infinite-scroll-distance="20">
            <li class="c_box item" v-for="(item, index) in handleOrderList">
              <router-link class="line head" :to="{path: '/storeOrder/detail', query: {orderCode: item.orderCode, isHideCancel: true}}">
              ...
              <div class="line eval" v-if="item.feedbackInfoList.length > 0">
                <div class="eval-head">
                  ...
                  <div class="eval-left"><span>评价信息：{{item.feedbackInfoList.length}}条</span></div>
        ...
        <bm-loadmore class="loadmore" v-scrolled="'storeOrderLiteHistory'" :top-method="loadList" ref="loadList_2" v-show="tab === 'HISTORY'">
          <ul
              v-infinite-scroll="fetchHistoryOrderList"
              infinite-scroll-disabled="historyOrderListScrollDisabled"
              infinite-scroll-distance="20">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        tab: 'storeOrder/liteTab',
        handleOrderList: 'storeOrder/liteHandleOrderList',
        historyOrderList: 'storeOrder/liteHistoryOrderList',
        handleOrderData: 'storeOrder/liteHandleOrderData',
        historyOrderData: 'storeOrder/liteHistoryOrderData',
        handleOrderLoaded: 'storeOrder/liteHandleOrderLoaded',
        historyOrderLoaded: 'storeOrder/liteHistoryOrderLoaded',
        loading: 'storeOrder/liteLoading'
      }),
      handleOrderListScrollDisabled () {
        return this.loading || this.handleOrderLoaded
      },
      historyOrderListScrollDisabled () {
        return this.loading || this.historyOrderLoaded
      }
    },
    methods: {
      doDecideToUseCache () {
        let storeId = this.$route.query['storeId']
        let cacheId = this.$store.state['storeOrder']['liteStoreId']
        if (storeId !== cacheId) {
          this.$store.dispatch('storeOrder/cleanLiteCache')
          this.$store.state['storeOrder']['liteStoreId'] = storeId
        }
      },
      // fetchList () {
      //   if (this.tab === 'STORE') {
      //     return this.$store.dispatch('storeOrder/liteFetchHandleOrderList')
      //   } else {
      //     return this.$store.dispatch('storeOrder/liteFetchHistoryOrderList')
      //   }
      // },
      async fetchHandleOrderList () {
        return await this.$store.dispatch('storeOrder/liteFetchHandleOrderList')
      },
      async fetchHistoryOrderList () {
        return await this.$store.dispatch('storeOrder/liteFetchHistoryOrderList')
      },
      // loadMore () {
      //   this.fetchList()
      // },
      async loadList () {
        let res
        if (this.tab === 'STORE') {
          res = await this.$store.dispatch('storeOrder/liteFetchHandleOrderList', true).catch(err => {
            this.$refs.loadList_1.onTopLoaded()
            throw err
          })
          this.$refs.loadList_1.onTopLoaded()
        } else {
          res = await this.$store.dispatch('storeOrder/liteFetchHistoryOrderList', true).catch(err => {
            this.$refs.loadList_2.onTopLoaded()
            throw err
          })
          this.$refs.loadList_2.onTopLoaded()
        }
        return res
        // this.$store.dispatch('storeOrder/liteResetStatusData')
        // this.fetchList().then(() => {
      ...
      operaFold (index, type) {
        if (type === 'STORE') {
          this.$store.commit('storeOrder/liteHandleFold', index)
        } else {
          this.$store.commit('storeOrder/liteHistoryFold', index)
        }
        this.$forceUpdate()
      },
      changeList (type) {
        this.$store.state['storeOrder']['liteTab'] = type
        // this.$store.dispatch('storeOrder/liteResetData')
        // this.fetchList()
      },
      // comment () {
      ...
      // },
      filterJustDate () {
        this.$store.state['storeOrder']['liteFilterDate'] = this.datePicker
        this.$store.dispatch('storeOrder/liteFetchHistoryOrderList', true)
        // this.fetchList()
      }
      // search () {
      //   this.datePicker = this.popupDatePicker
      //   this.$store.state['storeOrder']['liteFilterDate'] = this.datePicker
      //   this.$store.state['storeOrder']['liteFilterStore'] = this.storeKey
      //   this.$store.dispatch('storeOrder/liteFetchHistoryOrderList', true)
      //   this.$store.dispatch('storeOrder/liteResetData')
      //   this.fetchList()
      ...
      //   this.storeKey = ''
      //   this.filtering = false
      //   this.$store.state['storeOrder']['']
      //   this.$store.dispatch('storeOrder/liteResetData')
      //   this.$store.dispatch('storeOrder/liteResetSearch')
      //   this.fetchList()
      // }
      // getBook () {
      ...
      // }
      // openBook () {
      //   if (global.appInfo && global.appInfo.client === 'ios') {
      //     webview({
      //       url: this.book,
      //       title: '操作手册',
      //       isNav: true
    ...
    created () {
      this.$store.state['storeOrder']['liteStoreId'] = this.$route.query['storeId']
    },
    mounted () {
      // this.getBook()
      if (!this.tab) this.$store.state['storeOrder']['liteTab'] = 'STORE'
      if (!this.isBackPage()) {
        if (this.$route.query.tab) this.changeList(this.$route.query.tab) // 通过url传参形式显示默认tab
        if (+this.$route.query['historyDate'] >= 0) {
          this.datePicker = new Date(+this.$route.query['historyDate'])
          this.filterJustDate()
        }
        this.$store.dispatch('storeOrder/liteResetData')
        this.resetPageKey()
