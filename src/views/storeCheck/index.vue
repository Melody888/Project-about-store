<template>
  <bm-layout>
    <div slot="header">
      <bm-header>今日盘库
      <div slot="right" style="margin-right: 0.12rem;font-size: 0.14rem" @click="$router.push('/storeCheck/selectPkStore')">新增盘库</div>
      </bm-header>
    </div>
          ...
          <li class="list-item" v-for="item in storePkList">
            <div class="up-part">
              <div class="upp-1 flex justify-between">
                ...
                <div>
                  <span class="color-999" style="margin-right: 0.1rem">{{item.planDate | datetime('date')}}</span><span class="color-999" @click="$router.push({path: '/storeCheck/detail', query: {orderNo: item.orderNo, storeId: item.storeId}})">详情</span>
            ...
            <div class="down-part flex justify-between nowrap">
              ...
              <div class="opera color-666" @click="$router.push({path: '/storeCheck/inputInfo', query: {orderNo: item.orderNo, storeId: item.storeId}})">编辑</div>
            </div>
          </li>
        </ul>
      </bm-loadmore>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        storePkList: 'storeCheck/storePkList/storePkList',
        loading: 'storeCheck/storePkList/loading',
        loaded: 'storeCheck/storePkList/loaded'
      }),
      success () {
    ...
    methods: {
      go (type) {
        if (type === 'today') {
          ...
          this.$router.replace('/storeCheck/history')
        }
      },
      search () {
        this.reload()
      ...
      fetchList () {
        return this.$store.dispatch('storeCheck/storePkList/fetchStorePkList')
      },
      loadMore () {
        this.fetchList()
      },
      reload () {
        this.$store.dispatch('storeCheck/storePkList/resetStatusData')
      },
      loadList () {
        this.$store.dispatch('storeCheck/storePkList/resetStatusData')
        this.fetchList().then(() => {
    ...
    created () {
      if (!this.isBackPage()) {
        this.$store.dispatch('storeCheck/storePkList/resetData')
        this.resetPageKey()
      }
    }
  }
