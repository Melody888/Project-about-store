...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        storePkList: 'storeCheck/historyList/storePkList',
        loading: 'storeCheck/historyList/loading',
        loaded: 'storeCheck/historyList/loaded'
      }),
      keyword: {
        set (val) {
          this.$store.commit('storeCheck/historyList/keyword', val)
        },
        get () {
          return this.$store.getters['storeCheck/historyList/keyword']
        }
      },
      success () {
    ...
    methods: {
      ...
      fetchList () {
        return this.$store.dispatch('storeCheck/historyList/fetchStorePkList')
      },
      loadMore () {
        this.fetchList()
      },
      reload () {
        this.$store.dispatch('storeCheck/historyList/resetStatusData')
      },
      loadList () {
        this.$store.dispatch('storeCheck/historyList/resetStatusData')
        this.fetchList().then(() => {
    ...
    created () {
      if (!this.isBackPage()) {
        this.$store.dispatch('storeCheck/historyList/resetData')
        this.resetPageKey()
      }
    }
  }
