  ...
  <bm-layout>
    <header slot="header">
      ...
      <form class="m_search" @submit.prevent="doSearch">
        <div class="wrap">
          <input class="input" type="text" v-model="keyword" placeholder="输入名称搜索" @change="doUpdateStoreKeywordForDisplay">
          <button class="reset" v-if="keyword" type="reset" @click="doResetKeyword"></button>
        </div>
        <button class="submit" type="submit">搜索</button>
      </form>
    </header>
    <mt-loadmore class="m_list" :top-method="doRefresh" ref="loadmore" v-scrolled="'showcasePatrolIndex'">
      <ul>
        <li class="item" v-for="(item, index) of list">
          <router-link class="wrap" :to="`detail/${item.code}`" append>
    ...
    <button class="m_add" @click="doGoAdd">新增<br>巡检单</button>
...
<script>
  ...
  export default {
    ...
    computed: {
      list () {
        return this.$store.state['showcasePatrol']['indexList'] || []
      }
    },
    methods: {
      doInit () {
        this.keyword = this.$store.state['showcasePatrol']['indexKeywordForDisplay']
        if (!this.list.length) this.$store.dispatch('showcasePatrol/indexGetList')
      },
      doUpdateStoreKeywordForDisplay () {
        this.$store.state['showcasePatrol']['indexKeywordForDisplay'] = this.keyword
      },
      doUpdateStoreKeyword () {
        this.$store.state['showcasePatrol']['indexKeyword'] = this.keyword
      },
      doOpenReply (index) {
      ...
      doResetKeyword () {
        this.keyword = ''
        this.doUpdateStoreKeywordForDisplay()
      },
      async doSearch () {
        this.doUpdateStoreKeyword()
        return await this.$store.dispatch('showcasePatrol/indexGetList')
      },
      async doRefresh () {
